...
public class MessageService : IMessageService
{
    ...
    public async Task<IEnumerable<Message>> GetChatMessagesAsync(Guid chatId, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            var messages = await _hubConnection.InvokeAsync<IEnumerable<Message>>(
        ...
        {
            System.Diagnostics.Debug.WriteLine($"Ошибка получения сообщений: {ex.Message}");
            return Enumerable.Empty<Message>();
        }
    }

    public async Task<Message?> SendMessageAsync(Guid chatId, string messageText, CancellationToken cancellationToken = default)
    {
        try
        {
            if (_hubConnection == null || _hubConnection.State != HubConnectionState.Connected)
            ...
            var message = await _hubConnection.InvokeAsync<Message>(
                "SendMessageAsync",
                chatId,
                messageText,
                cancellationToken);
    ...
    private void RegisterEventHandlers()
    {
        ...
        _hubConnection.On<Message>(HubConstants.MessageEvents.MessageReceived, message =>
        {
            OnMessageReceived?.Invoke(message);
        });

        _hubConnection.On<Message>(HubConstants.MessageEvents.MessageSent, message =>
        {
            OnMessageSent?.Invoke(message);
        });

        _hubConnection.On<Guid>(HubConstants.MessageEvents.MessageDeleted, messageId =>
        {
            OnMessageDeleted?.Invoke(messageId);
        });

        _hubConnection.On<Message>(HubConstants.MessageEvents.MessageUpdated, message =>
        {
